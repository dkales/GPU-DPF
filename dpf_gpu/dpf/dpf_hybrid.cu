...
void dpf_hybrid_initialize(int batch_size, int num_entries) {
  size_t n_bytes = sizeof(uint128_t_gpu) * batch_size *
                   ((int)ceil(log2((float)num_entries))) * Z * 2;
...
__global__ void dpf_hybrid_kernel(SeedsCodewordsFlatGPU *cw, uint128_t_gpu *out,
                                  ...
                                  uint128_t_gpu **TABLE, int batch_size,
                                  int num_entries) {
  ...
  constexpr int parallel_work_per_threadblock_per_dpf =
      (DPF_HYBRID_THREADS_PER_BLOCK / DPF_HYBRID_DPFS_PER_BLOCK);
  ...
  key_write[0 + (block_idx + batch_idx) * max_stack_size_per_dpf] =
      cw_shared[batch_idx].last_keys[0];
  ...
  for (int i = cw_shared[0].depth - 1; i >= cw_shared[0].depth - LOGZ; i--) {
    ...
    // Can parallelize _within_ a level of the tree, but not _across_ levels of
    // the tree
    __syncthreads();
  ...
  for (int j = prev_start; j < prev_end;
       j += parallel_work_per_threadblock_per_dpf) {
    int expansion_idx =
        j + (thread_idx % parallel_work_per_threadblock_per_dpf);
    int batch_idx = thread_idx / parallel_work_per_threadblock_per_dpf;

    if (expansion_idx < prev_end) {
      key_read[expansion_idx +
               (block_idx + batch_idx) * max_stack_size_per_dpf] =
          key_write[expansion_idx +
                    (block_idx + batch_idx) * max_stack_size_per_dpf];
    }
  ...
  while (stack_indx >= 0) {
    ...
    if (cur_depth == cw_shared[0].depth) {
      ...
      for (int i = 0; i < Z; i += parallel_work_per_threadblock_per_dpf) {
        int expansion_idx =
            i + (thread_idx % parallel_work_per_threadblock_per_dpf);
        int batch_idx = thread_idx / parallel_work_per_threadblock_per_dpf;

        if (expansion_idx < Z) {

#if (FUSES_MATMUL == 0)
          // This writes the full expanded DPF output (non-fused) in a permuted
          // ordering
          out[write_out + expansion_idx + batch_idx * num_entries +
              block_idx * DPF_HYBRID_DPFS_PER_BLOCK * num_entries] =
              key_read[cw_read_write_indx_strt + expansion_idx +
                       (block_idx + batch_idx) * max_stack_size_per_dpf];
#else
          // This accumulates, per-thread, partial dot product of table against
          // DPF output
          ...
          for (int z = 0; z < MM; z++) {
            per_thread_accumulate[z] = add_uint128(
                mul_uint128(dpf_output,
                            TABLE[batch_idx]
                                 [write_out + expansion_idx + z * num_entries]),
      ...
      for (int j = start; j < end; j += parallel_work_per_threadblock_per_dpf) {
        ...
        if (expansion_idx < end) {
          ...
          // We could try to load these into shared memory to reduce reads by
          // 1/2. Tried this and it didn't help. Likely because primarily
          // compute bound.
          uint128_t_gpu key =
              key_read[cw_read_write_indx_strt + (expansion_idx / 2) +
                       (block_idx + batch_idx) * max_stack_size_per_dpf];
          uint128_t_gpu value = PRF<prf_method>(key, idx_into_codewords);
          uint128_t_gpu *cw = (key.x & 1) == 0 ? cw_shared[batch_idx].cw_1
                                               : cw_shared[batch_idx].cw_2;
  ...
  for (int element = 0; element < MM; element++) {
    ...
    // Tree sum
    for (int neighbor = 1; neighbor < DPF_HYBRID_THREADS_PER_BLOCK;
         neighbor *= 2) {
      if (thread_idx % (neighbor * 2) == 0 &&
          thread_idx + neighbor < DPF_HYBRID_THREADS_PER_BLOCK) {
        shared_accumulates[thread_idx] =
            add_uint128(shared_accumulates[thread_idx],
...
template <int prf_method>
void dpf_hybrid(SeedsCodewordsFlatGPU *cw, uint128_t_gpu *out,
                uint128_t_gpu **TABLE, int batch_size, int num_entries,
                cudaStream_t s) {
  dim3 n_blocks(batch_size / DPF_HYBRID_DPFS_PER_BLOCK);
  dim3 n_threads(DPF_HYBRID_THREADS_PER_BLOCK);
  dpf_hybrid_kernel<prf_method><<<n_blocks, n_threads, 0, s>>>(
      cw, out, DPF_HYBRID_STACK_1, DPF_HYBRID_STACK_2, TABLE, batch_size,
      num_entries);
}
