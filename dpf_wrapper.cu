...
std::vector<void *> eval_init(at::Tensor table) {
  ...
  uint128_t_gpu *table_reordered_cvted =
      new uint128_t_gpu[num_entries * entry_size];
  for (int j = 0; j < entry_size; j++) {
    for (int i = 0; i < num_entries; i++) {
      int reordered_indx = brev_cpu(i) >> 32 - (int)log2(num_entries);
      table_reordered_cvted[i + j * num_entries] =
          uint128_gpu_from((uint128_t)table[reordered_indx][j].item<int>());
    }
  }

  uint128_t_gpu **TABLE;

  gpuErrchk(cudaMalloc(&TABLE, sizeof(uint128_t *) * BATCH_SIZE));
  ...
  gpuErrchk(
      cudaMalloc((void **)&CW_GPU, sizeof(SeedsCodewordsFlatGPU) * BATCH_SIZE));
...
at::Tensor eval_gpu(std::vector<at::Tensor> keys, std::vector<void *> buffers,
                    int n, int prf_method) {
  assert(keys.size() == BATCH_SIZE);

  SeedsCodewordsFlatGPU *cw_intermediate = (SeedsCodewordsFlatGPU *)malloc(
      sizeof(SeedsCodewordsFlatGPU) * BATCH_SIZE);
  ...
  cudaMemcpy(out_cpu, OUT, sizeof(uint128_t_gpu) * BATCH_SIZE * MM,
